...
#+BEGIN_SRC bash :tangle no
  # download emacs version 30.2
  export EMACS_VERSION="30.2"
...
This config is organized Prelude-style (`https://github.com/bbatsov/prelude'):

+ ~init.el~ bootstraps ~straight.el~/~use-package~ and loads everything else.
+ ~core/~ holds config that's always loaded: completion, UI/theme, editor
  defaults, and general programming tooling (Eglot, Flycheck, Projectile,
  Treemacs, Magit, etc.). Not optional.
+ ~modules/~ holds one file per language/topic (F#, Python, Go, org-mode,
  ...). Enable or disable a module by (un)commenting its entry in
  ~modules/modules-list.el~ -- no need to delete the file.
+ ~yasnippets/~ holds snippet templates, unchanged by the file layout above.

Each file documents its own packages and common commands in its
~;;; Commentary:~ header -- that's the reference, no separate docs folder.
...
I use this feature to tell Emacs where to find this project by adding this line
of code to the beginning of ~~/.emacs.d/init.el~:

#+BEGIN_SRC elisp
  ;; ~/.emacs.d/init.el
  (load-file "~/Desktop/codes/emacs-config/init.el")
#+END_SRC
...
+ [[https://www.gnu.org/software/emacs/manual/html_node/eglot/][Eglot manual - the built-in LSP client this config uses]]
